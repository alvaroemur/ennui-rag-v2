"""
Sidebar de Retrieval del programa - SOLUCIÓN INMEDIATA
"""
...
def render_retrieval_screen():
    ...
    render_diagnostic_info(selected_program_id, program_data, indexing_status, file_stats)
...
def render_diagnostic_info(program_id, program_data, indexing_status, file_stats):
    """Renderiza información de diagnóstico"""
    st.markdown("#### 🔧 Información de Diagnóstico")
    
    with st.expander("Ver detalles técnicos", expanded=False):
        st.markdown("**Programa ID:**", program_id)
        st.markdown("**Programa Data:**", program_data)
        st.markdown("**Indexing Status:**", indexing_status)
        st.markdown("**File Stats:**", file_stats)
        
        # Verificar estado del backend
        backend_health = check_backend_health()
        st.markdown(f"**Backend Health:** {'✅ OK' if backend_health else '❌ Error'}")
        ...
        try:
            ...
            if resp.status_code == 200:
                jobs = resp.json()
                st.markdown(f"**Jobs en BD:** {len(jobs)}")
                for i, job in enumerate(jobs[:3]):  # Mostrar solo los primeros 3
                    st.markdown(f"  - Job {i+1}: ID={job.get('id')}, Status={job.get('status')}, Created={job.get('created_at')}")
            else:
                st.error(f"Error al obtener jobs: {resp.status_code}")
...
def render_indexing_status_details(program_id, indexing_status, file_stats):
    ...
    if indexing_status:
        ...
        if status == "running":
            ...
            # Verificar si está pegado (más de 5 minutos sin progreso)
            started_at = indexing_status.get("started_at")
            if started_at:
                try:
                    start_time = datetime.fromisoformat(started_at.replace('Z', '+00:00'))
                    elapsed = datetime.utcnow() - start_time.replace(tzinfo=None)
                    if elapsed.total_seconds() > 300:  # 5 minutos
                        st.warning("⚠️ **Indexación parece estar pegada** (más de 5 minutos sin progreso)")
                        if st.button("🔧 Resetear jobs pegados", key="reset_stuck_jobs"):
                            reset_stuck_jobs(program_id)
                except:
                    pass
        ...
        # Botón para forzar actualización del estado
        if st.button("🔄 Forzar actualización del estado", key="force_refresh_status"):
            st.rerun()
...
def reset_stuck_jobs(program_id):
    """Resetea jobs de indexación pegados"""
    try:
        # Llamar al endpoint de reset (si existe)
        resp = make_authenticated_request(
            "POST", 
            f"{API_BASE_URL_INTERNAL}/api/indexing/reset-stuck-jobs/{program_id}", 
            timeout=10
        )
        
        if resp.status_code == 200:
            result = resp.json()
            st.success(f"✅ {result.get('message', 'Jobs reseteados')}")
            st.rerun()
        else:
            st.error("❌ Error al resetear jobs. Reinicia el backend manualmente.")
    except Exception as e:
        st.error(f"❌ Error de conexión: {str(e)}")
        st.info("💡 **Solución manual:** Reinicia el backend con `docker-compose restart backend`")
...
def render_indexing_buttons(program_id, indexing_status, file_stats):
    ...
    if is_indexing:
        ...
        # Botón para cancelar/resetear si está pegado
        col1, col2, col3 = st.columns([1, 1, 2])
        with col1:
            if st.button("🔄 Forzar actualización", key="force_refresh"):
                st.rerun()
        with col2:
            if st.button("⚠️ Resetear estado", key="reset_status", type="secondary"):
                reset_stuck_jobs(program_id)
    elif not is_indexed:
