...
# Main content
st.title("Welcome to ennui-rag-v2 de Alvaro Mur")
...
if st.session_state["jwt"] is not None:
    ...
    recent_posts = requests.get(f"{API_BASE_URL_INTERNAL}/api/posts/recent", headers=headers)
    ...
    if st.button("Submit"):
        headers = {"Authorization": f"Bearer {st.session_state['jwt']}"}
        response = requests.post(f"{API_BASE_URL_INTERNAL}/api/post", headers=headers, 
