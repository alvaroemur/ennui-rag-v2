"""
Sidebar de Retrieval del programa - VERSIÓN DE DIAGNÓSTICO
"""
    ...
    if is_indexing:
        ...
        with col2:
            if st.button("⚠️ Resetear estado", key="reset_status", type="secondary"):
                reset_indexing_status(program_id)
    elif not is_indexed:
...
def reset_indexing_status(program_id):
    """Resetea el estado de indexación si está pegado"""
    try:
        # Aquí podrías implementar un endpoint para resetear el estado
        st.warning("⚠️ Funcionalidad de reset pendiente de implementación")
        st.info("Por ahora, reinicia el backend para limpiar el estado")
    except Exception as e:
        st.error(f"Error al resetear estado: {str(e)}")
