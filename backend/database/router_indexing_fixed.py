...
from apps.indexing_service_fixed import IndexingService, process_indexing_job_background
...
async def get_file_stats(
    ...
    file_types = db.query(IndexedFile.file_type, db.func.count(IndexedFile.id)).filter(
        IndexedFile.program_id == program_id,
        IndexedFile.indexing_status == "completed"
    ).group_by(IndexedFile.file_type).all()
    
    # Tamaño total
    total_size = db.query(db.func.sum(IndexedFile.file_size)).filter(
        IndexedFile.program_id == program_id,
        IndexedFile.indexing_status == "completed"
    ).scalar() or 0
